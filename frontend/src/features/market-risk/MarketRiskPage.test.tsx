...
describe('FE-08 Market Risk Lab', () => {
  it('renders model diagnostics and explicit pending contracts', async () => {
    renderLab();

    expect(
      await screen.findByRole('heading', { name: 'Market Risk Lab' }),
    ).toBeInTheDocument();
    ...
    expect(screen.getAllByText(/MODEL CONTRACT PENDING/).length).toBeGreaterThanOrEqual(
      4,
    );
  });

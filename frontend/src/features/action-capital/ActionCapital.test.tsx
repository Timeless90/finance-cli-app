...
describe('FE-09 Action Steering & Capital Allocation', () => {
  it('renders action lifecycle and realized benefits', async () => {
    renderWorkspace('actions');
    expect(
      await screen.findByRole('heading', { name: 'Action Steering' }),
    ).toBeInTheDocument();
    expect(screen.getAllByText('ACT-042').length).toBeGreaterThan(0);

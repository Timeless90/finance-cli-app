...
> = {
  ...
  'local-upside': {
    ...
    briefing: {
      headline:
        'Upside conversion is credible if commercial execution stays on cadence.',
      summary:
  ...
  'local-downside': {
    metrics: [
      {
        ...baseSnapshot.metrics[0]!,
        value: '€451.7M',
        delta: '-2.6% vs plan',
        deltaTone: 'negative',
      },
      {
        ...baseSnapshot.metrics[1]!,
        value: '€68.9M',
        delta: '-€9.6M vs plan',
        deltaTone: 'negative',
      },
      ...
      {
        ...baseSnapshot.metrics[3]!,
        value: '€57.2M',
        delta: '€11.7M expected loss',
        deltaTone: 'negative',
      },
    ],
    liquidity: {
...
export function getMockCommandCenterSnapshot(
  context: CommandCenterContext,
): CommandCenterSnapshot {
  const scenario =
    scenarioOverrides[context.scenarioId] ?? scenarioOverrides['local-base']!;

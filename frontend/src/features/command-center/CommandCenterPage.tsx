...
function LoadingState() {
  return (
    <div
      className="grid gap-3 p-4 lg:grid-cols-4 lg:p-6"
      aria-label="Loading command center"
    >
      {Array.from({ length: 8 }, (_, index) => (
        <div
          className="h-36 animate-pulse border border-[var(--frame-muted)] bg-[var(--surface-panel)]"
          key={index}
        />
      ))}
    </div>
      ...
      <TacticalFrame label="COMMAND CENTER // DATA ERROR" tone="negative">
        <div className="p-6">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
            {describeApiError(error, 'Command Center')} No finance calculation is
  ...
  return (
    <div className="grid gap-4 p-4 lg:p-6">
      <section className="grid gap-4 xl:grid-cols-[1fr_auto] xl:items-end">
        <div>
          <div className="interface-label text-[var(--signal-primary)]">
            EXECUTIVE COCKPIT // FE-05
          </div>
          <h1 className="mt-2 font-[var(--font-display)] text-3xl font-semibold tracking-[-0.035em] sm:text-4xl">
            Command Center
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--text-secondary)]">
            Cross-module view of performance, liquidity, risk and management actions for{' '}
            {snapshot.context.companyLabel}.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <StatusIndicator
            label="COVERAGE"
            detail={snapshot.assurance.coverage}
            tone="positive"
          />
        </div>
      </section>
      ...
      {snapshot.contractStatus === 'MOCK_CONNECTED' && (
        <div className="border border-[var(--signal-warning)] bg-[color:oklch(0.82_0.15_85/0.06)] px-4 py-3">
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">
            Executive finance values are simulated frontend fixtures in mock mode.
          </p>
        </div>
      )}
      ...
      <section className="grid gap-4 2xl:grid-cols-[1.55fr_0.75fr]">
        ...
        >
          <div className="p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-5 border-t border-[var(--frame-muted)] pt-3">
              ...
              <span className="interface-label text-[var(--signal-positive)]">
                -- UPSIDE
              </span>
        ...
        <TacticalFrame
          label="LIQUIDITY ENVELOPE"
          tone={snapshot.liquidity.tone === 'positive' ? 'positive' : 'default'}
        >
          <div className="grid gap-px bg-[var(--frame-muted)] sm:grid-cols-2 2xl:grid-cols-1">
            {[
              ...
              <div className="bg-[var(--surface-panel)] p-4" key={label}>
                ...
                <div
                  className={`data-value mt-2 text-2xl ${signalClasses[snapshot.liquidity.tone]}`}
                >
                  {value}
                </div>
              </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2 2xl:grid-cols-[0.9fr_1.1fr_1fr]">
          ...
          <div className="divide-y divide-[var(--frame-muted)]">
            {snapshot.varianceDrivers.map((driver) => (
              ...
              >
                ...
                <span className={`data-value text-sm ${signalClasses[driver.tone]}`}>
                  {driver.amount}
                </span>
            ...
            {snapshot.risk.signals.map((risk) => (
              ...
              >
                ...
                <div className="text-right">
                  <div className="data-value text-sm text-[var(--signal-negative)]">
                    {risk.exposure}
                  </div>
                  <div className="data-value mt-1 text-[0.62rem] text-[var(--text-muted)]">
        ...
        <TacticalFrame label="ASSURANCE">
          <div className="grid gap-px bg-[var(--frame-muted)] sm:grid-cols-2 xl:grid-cols-1">
            <StatusIndicator
              label="FRESHNESS"
              detail={snapshot.assurance.dataFreshness}
              tone="positive"
            />
            <StatusIndicator
              label="DATA COVERAGE"
            ...
            <StatusIndicator
              label="MODEL"
              detail={snapshot.assurance.modelStatus}
              tone="positive"
            />
            <StatusIndicator
              label="LINEAGE"
              detail={snapshot.assurance.lineageStatus}
              tone="positive"
            />
          </div>
        </TacticalFrame>
            ...
            <div className="mt-5 border-t border-[var(--frame-muted)] pt-4">
              ...
              <ol className="mt-3 grid gap-3">
                {snapshot.briefing.decisions.map((decision, index) => (
                  <li
                    className="grid grid-cols-[1.5rem_1fr] gap-2 text-sm leading-5"
                    key={decision}
                  >

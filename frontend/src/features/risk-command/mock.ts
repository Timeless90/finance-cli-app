...
const baseRisks: RiskCommandSnapshot['risks'] = [
  ...
  {
    id: 'R-038',
    title: 'Cyber service disruption',
    category: 'Cyber',
    owner: 'CIO',
    probability: 0.18,
    impact: 0.92,
  ...
  {
    id: 'R-044',
    title: 'Liquidity refinancing spread',
    category: 'Liquidity',
    owner: 'CFO',
    probability: 0.24,
    impact: 0.66,
...
const distribution: RiskCommandSnapshot['portfolio']['distribution'] = [
  { percentile: 5, loss: 0.8 },
  { percentile: 10, loss: 1.2 },
  { percentile: 20, loss: 2.0 },
  { percentile: 30, loss: 2.8 },
  { percentile: 40, loss: 3.7 },
  { percentile: 50, loss: 4.6 },
  { percentile: 60, loss: 5.5 },
  { percentile: 70, loss: 6.7 },
  { percentile: 80, loss: 8.2 },
  { percentile: 90, loss: 11.7 },
  { percentile: 95, loss: 15.4 },
  { percentile: 97.5, loss: 18.8 },
  { percentile: 99, loss: 23.6 },
  { percentile: 99.5, loss: 27.9 },
];
...
const base: Omit<RiskCommandSnapshot, 'context' | 'contractStatus'> = {
  ...
  radar: categories.map((item) => ({
    ...
    exposure: item.residualExposure,
  ...
  tail: {
    ...
    qq: [
      { theoretical: 0.5, observed: 0.6 },
      { theoretical: 1.1, observed: 1.0 },
      { theoretical: 1.8, observed: 1.9 },
      { theoretical: 2.7, observed: 2.6 },
      { theoretical: 3.9, observed: 4.2 },
      { theoretical: 5.4, observed: 5.9 },
      { theoretical: 7.2, observed: 8.0 },
      { theoretical: 9.6, observed: 11.1 },
      { theoretical: 12.8, observed: 15.4 },
    ],
  },
  scenario: {
    name: 'Energy + demand compression',
    description:
      'Simultaneous energy escalation, DACH volume softness and FX translation pressure.',
  ...
  controls: [
    ...
    },
    {
      id: 'CTL-09-A',
      riskId: 'R-009',
      name: 'Pricing and demand trigger',
    ...
    },
    {
      id: 'CTL-31-B',
      riskId: 'R-031',
      name: 'Dual-source qualification',
      owner: 'Procurement',
      effectiveness: '24%',
      annualCost: '€0.16M',
      avoidedLoss: '€0.2M',
...
export function getMockRiskCommandSnapshot(
  ...
  if (selection.scenarioId === 'local-downside') {
    return {
      ...
      portfolio: {
        ...
        p95NetLoss: '€28.6M',
        p99NetLoss: '€43.9M',
        expectedShortfall95: '€36.7M',
        appetiteUsage: '88%',
      },
      risks: base.risks.map((risk) => ({
        ...risk,
        probability: Math.min(0.95, risk.probability + 0.12),
        appetiteUsage:
          risk.id === 'R-017'
            ? '112%'
            : risk.id === 'R-009'
              ? '104%'
              : risk.appetiteUsage,
        status: risk.id === 'R-017' || risk.id === 'R-009' ? 'BREACHED' : risk.status,
      })),
      scenario: {
        ...base.scenario,
        name: 'Combined downside',
        earningsAtRisk: '€24.9M EBITDA',
        cashAtRisk: '€22.1M FCF',
        probability: '29%',
      },
    };
  }
  if (selection.scenarioId === 'local-upside') {
    return {
      ...
      portfolio: {
        ...
        p95NetLoss: '€11.9M',
        p99NetLoss: '€18.2M',
        expectedShortfall95: '€15.6M',
        appetiteUsage: '49%',
      },
      scenario: {
        ...base.scenario,
        name: 'Protected upside',
        earningsAtRisk: '€9.1M EBITDA',
        cashAtRisk: '€6.8M FCF',
        probability: '11%',
      },
    };

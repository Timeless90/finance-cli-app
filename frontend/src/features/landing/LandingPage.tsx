...
const features = [
  {
    ...
    description:
      'Driver-based planning, rolling forecasts and probabilistic outlooks in one command layer.',
    signal: 'P10 / P50 / P90',
  },
  {
    code: '02',
    title: 'Risk Command',
    description:
      'Connect enterprise, liquidity and market risk to the financial plan and decision horizon.',
  ...
  },
  {
    ...
    title: 'Scenario Lab',
    description:
      'Compare base, upside and downside paths without losing model, assumption or run lineage.',
  ...
  },
  {
    ...
    title: 'AI Briefings',
    description:
      'Turn governed finance outputs into evidence-linked explanations, actions and management narratives.',
    signal: 'EXPLAIN / ACT',
  },
];
...
function BrandMark() {
  return (
    <Link
      aria-label="CFO Command Center home"
    ...
    >
      ...
      <span>
        <span className="interface-label block text-[var(--signal-primary)]">
          COMMAND CENTER
        </span>
        <span className="data-value mt-0.5 block text-[0.6rem] text-[var(--text-muted)]">
          FINANCE OS // 2060
        </span>
      </span>
    </Link>
  ...
  return (
    <div className="relative overflow-hidden border border-[var(--frame-muted)] bg-[var(--surface-canvas)] p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="interface-label m-0 text-[var(--text-muted)]">
            12M PROBABILISTIC OUTLOOK
          </p>
          <p className="data-value m-0 mt-2 text-lg text-[var(--signal-primary)]">
            EBITDA // UI PREVIEW
          </p>
        </div>
        ...
        <div className="grid gap-px bg-[var(--frame-muted)] lg:grid-cols-[1fr_14rem]">
          ...
          <aside className="grid content-start gap-px bg-[var(--frame-muted)]">
            <div className="bg-[var(--surface-panel)] p-4">
              ...
              <p className="data-value m-0 mt-2 text-xl text-[var(--signal-primary)]">
                BASE // FY26
              </p>
            </div>
            ...
            <div className="bg-[var(--surface-panel)] p-4">
              ...
              <p className="mt-3 text-xs leading-5 text-[var(--text-secondary)]">
                Margin pressure remains concentrated in volume and energy cost drivers.
                No live analysis is executed on this public preview.
              </p>
            </div>
          </aside>
        </div>
...
export function LandingPage() {
  return (
    <main className="ds-environment min-h-screen overflow-hidden bg-[var(--surface-canvas)] text-[var(--text-primary)]">
        ...
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-5 px-4 py-3 sm:px-6 lg:px-10">
          ...
          >
            {[
              ...
              <a
                className="interface-label text-[var(--text-secondary)] no-underline transition-colors hover:text-[var(--signal-primary)]"
                href={href}
                key={href}
              >
                {label}
      ...
      <section className="relative mx-auto grid min-h-[82vh] max-w-[1500px] items-center gap-12 px-4 py-20 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-10 lg:py-28">
        <div className="relative z-10">
          <div className="mb-6 flex items-center gap-3">
            ...
            <p className="interface-label m-0 text-[var(--signal-primary)]">
              FINANCE INTELLIGENCE // NEXT HORIZON
            </p>
          </div>
          <h1 className="m-0 max-w-4xl font-display text-[clamp(3.4rem,7vw,7.7rem)] font-semibold uppercase leading-[0.82] tracking-[-0.065em]">
            See the future{' '}
            <span className="text-[var(--signal-primary)]">of finance.</span>
          </h1>
          <p className="mt-8 max-w-xl text-base leading-7 text-[var(--text-secondary)] sm:text-lg">
            One command layer for forecasting, performance, liquidity, risk, capital
            allocation and governed AI insight.
          </p>
          ...
          <div className="mt-10 grid max-w-xl gap-px bg-[var(--frame-muted)] sm:grid-cols-3">
            {[
              ...
              <div className="bg-[var(--surface-panel)] p-3" key={label}>
                ...
                <p className="data-value m-0 mt-1 text-xs text-[var(--signal-primary)]">
                  {value}
                </p>
              </div>
            ))}
      ...
      <section
        aria-label="Simulated product metrics"
        className="border-y border-[var(--frame-muted)] bg-[var(--surface-panel)]"
      >
          ...
          {previewMetrics.map((metric) => (
            <div className="p-5 lg:p-7" key={metric.label}>
              ...
              <div className="mt-3 flex items-end justify-between gap-4">
                <p className="data-value m-0 text-2xl text-[var(--signal-primary)] sm:text-3xl">
                  {metric.value}
                </p>
                <span className="data-value text-[0.65rem] text-[var(--signal-positive)]">
                  {metric.detail}
                </span>
              </div>
              <p className="data-value m-0 mt-3 text-[0.58rem] text-[var(--text-muted)]">
                SIMULATED UI PREVIEW // NOT LIVE DATA
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
          <div>
            ...
            <p className="mt-6 max-w-md text-sm leading-6 text-[var(--text-secondary)]">
              The visual system stays cinematic. The underlying product stays
              disciplined: governed models, explicit scenarios, traceable outputs and
              clear action paths.
            </p>
          </div>
          <div className="grid gap-px bg-[var(--frame-muted)] sm:grid-cols-2">
            {features.map((feature) => (
              <article
                className="group min-h-64 bg-[var(--surface-panel)] p-6 transition-colors hover:bg-[var(--surface-panel-raised)]"
                id={
                  feature.code === '01'
                    ? 'forecasting'
                    : feature.code === '02'
              ...
              >
                ...
                <h3 className="mt-14 font-display text-2xl font-semibold uppercase tracking-[-0.03em]">
                  {feature.title}
                </h3>
      ...
      <section className="border-y border-[var(--frame-muted)] bg-[var(--surface-panel)]">
        <div className="mx-auto grid max-w-[1500px] gap-8 px-4 py-20 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-10 lg:py-24">
          <div>
            <p className="interface-label m-0 text-[var(--signal-primary)]">
              CONTROL SURFACE // GOVERNED BY DESIGN
            </p>
          ...
          <div className="grid gap-px bg-[var(--frame-muted)] sm:grid-cols-2">
            {[
              [
                '01',
                'OPENAPI CONTRACT',
                'Frontend types follow backend Pydantic models instead of duplicating them.',
              ],
              [
                '02',
                'RUN LINEAGE',
                'Finance outcomes remain attributable to scenarios, models, snapshots and governed runs.',
              ],
              [
                '03',
                'MOCK / LIVE',
                'Frontend development continues in parallel without pretending mock data is production data.',
              ],
              [
                '04',
                'ACCESSIBLE UI',
                'Technical density remains keyboard-operable, readable and responsive.',
              ],
            ].map(([code, title, description]) => (
              <div className="bg-[var(--surface-canvas)] p-5" key={code}>
                <p className="data-value m-0 text-[0.65rem] text-[var(--signal-primary)]">
                  {code}
                </p>
                <p className="interface-label m-0 mt-8 text-[var(--text-primary)]">
                  {title}
                </p>
                <p className="mt-3 text-xs leading-5 text-[var(--text-secondary)]">
                  {description}
      ...
      <footer className="border-t border-[var(--frame-muted)]">
        <div className="mx-auto flex max-w-[1500px] flex-col gap-4 px-4 py-6 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-10">
          <BrandMark />
          <p className="data-value m-0 text-[0.6rem] text-[var(--text-muted)]">
            PUBLIC LANDING // NO BUSINESS API DEPENDENCY // FE-04
          </p>
        </div>
      </footer>
    </main>

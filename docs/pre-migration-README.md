# Historical application guide (before template migration)

The commands below are historical. Use the root README for current development.
...
> **Frontend status:** `frontend/` contains the active React/Vite CFO Command Center. It is a local/UAT application; production identity and cloud services are deliberately not configured in this repository.
...
Run it locally without persistence:
...
### Dedicated local Finance Platform container

The local stack is isolated from other projects: `finance-app-postgres` owns its own
`finance-app-postgres-data` Docker volume and belongs only to `finance-app-network` with
the `finance-app-server` API. It runs non-personal UAT projections, uses the local
`developer` identity at the Vite gateway, writes governed runs to the `finance_app_dev`
database, and uses an echo model instead of Azure Foundry.

```bash
# One-time local configuration (the real file is Git-ignored)
cp .env.local.example .env.local
cp frontend/.env.local.example frontend/.env.local

# Build the API image, then create the isolated database once
docker build -t finance-platform:local .
docker network create finance-app-network
docker volume create finance-app-postgres-data
docker volume create finance-app-imports-data
docker run --detach --name finance-app-postgres \
  --network finance-app-network --publish 127.0.0.1:5433:5432 \
  --restart unless-stopped \
  --env POSTGRES_USER=finance_app_dev \
  --env POSTGRES_PASSWORD=finance-app-local-dev \
  --env POSTGRES_DB=finance_app_dev \
  --volume finance-app-postgres-data:/var/lib/postgresql/data \
  postgres:17-alpine
docker run --detach --name finance-app-server \
  --network finance-app-network --publish 8000:8000 --restart unless-stopped \
  --volume finance-app-imports-data:/app/data/imports \
  --env CFO_ENVIRONMENT=uat \
  --env CFO_GOVERNANCE_DATABASE_URL=postgresql://finance_app_dev:finance-app-local-dev@finance-app-postgres:5432/finance_app_dev \
  --env CFO_IMPORT_STORAGE_PATH=/app/data/imports \
  --env CFO_FOUNDRY_MODE=echo \
  --env CFO_RATE_LIMIT_REQUESTS_PER_MINUTE=0 \
  finance-platform:local

# Start the local developer gateway and browser client
cd frontend
pnpm dev:local
```

Open `http://localhost:5173`; API health is at `http://localhost:8000/health/ready`.
The developer gateway owns the trusted headers—do not add them from browser code.

### Local governed data import

Open **Data & Governance** in the frontend to load a CSV/XLSX file. The import accepts
the canonical columns `company`, `account`, `period`, `scenario`, `value`, and `currency`;
optional dimensions use the `dim_` prefix. A mapping JSON can translate source-system
column names to those canonical names.

The local Vite gateway exposes only three fixed test profiles, never arbitrary browser
roles: **Developer / Preparer** uploads, **Local Reviewer** reviews, and **Local Approver**
approves and publishes. The transition creates durable PostgreSQL import metadata and
content-addressed snapshots; the original source file is kept in
`finance-app-imports-data`. Publication generates context-bound Planning, Performance,
Profitability, Liquidity, Command Center and Data & Governance read models with snapshot
lineage. Other workspaces deliberately receive an empty published model rather than
invented financial values.

### Local Planning baseline

After publishing an import, select it under **Data & Governance → Planning Baseline /
Account Mapping**. Assign every source account to a Planning category and set the sign
normalization. The initial suggestions treat accounts beginning with `4` as revenue,
`5` as variable cost, `6` as fixed operating cost, and `1000` as cash; review every
suggestion against the source chart of accounts.

Create the mapping draft as **Developer / Preparer**, review it as **Local Reviewer**,
approve it as **Local Approver**, then publish the baseline for the active
company/period/scenario. A baseline is forecast-eligible only when it contains revenue,
variable cost and fixed operating cost. The Planning workspace displays the published
snapshot, mapping version and eligibility state. This local segregation is a workflow
test; production approval must use separate authenticated identities.

To stop the isolated local stack, use
`docker stop finance-app-server finance-app-postgres`. The PostgreSQL data remains in its
dedicated Docker volume; start the same two containers to resume it.

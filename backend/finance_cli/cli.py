...
from finance_cli.calibration import CalibrationResult, calibrate
from finance_cli.charts import export_percentile_chart
from finance_cli.data import load_price_csv, to_monthly_returns
from finance_cli.diagnostics import compare_distribution_fits, rolling_origin_backtest
from finance_cli.models import AppConfig, SimulationMethod
from finance_cli.reporting import (
    build_horizon_summary,
    build_path_percentiles,
    export_results,
)
from finance_cli.risk import path_risk_metrics, summarize_risk_metrics
...
def simulate(
    ...
    after_tax, taxes = apply_terminal_tax(
        output.values[:, -1], output.paid_in[-1], tax_policy
    )
    tax_summary = {
    ...
    if simple_returns is not None and cfg.diagnostics.enabled:
        assert historical_log_returns is not None
...
def wizard(
    ...
    config.portfolio.monthly_contribution = typer.prompt(
        ...
        default=config.portfolio.monthly_contribution,
        type=float,
    )
    ...
    config.tax.enabled = typer.confirm(
        "Enable simplified German tax model?", default=False
    )
    output.write_text(config.model_dump_json(indent=2), encoding="utf-8")

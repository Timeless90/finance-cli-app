...
class TemplateRegistry:
    ...
    def register(self, template: ReportTemplate) -> None:
        ...
        if key in self._templates:
            raise ValueError(
                f"template already exists: {template.template_id} v{template.version}"
            )
...
class ReportingFactory:
    def __init__(
        self, registry: TemplateRegistry, repository: InMemoryReportRepository
    ) -> None:
    ...
    def generate_from_published_sources(
        self,
        template_id: str,
        template_version: int,
        source_snapshot_ids: tuple[str, ...],
    ) -> ReportArtifact:
        """Create a UAT report shell whose lineage is bound on the server.

        This deliberately does not accept displayed browser values. Production
        source assembly is replaced by persisted projections in Release 7.
        """
        if not source_snapshot_ids:
            raise ValueError("at least one source snapshot is required")
        template = self._registry.get(template_id, template_version)
        source_refs = tuple(source_snapshot_ids)
        sections = tuple(
            ReportSection(
                ...
                values=(
                    ReportValue(
                        key=f"{section_id}-source-binding",
                        value="Published backend source binding",
                        unit=None,
                        snapshot_id=source_snapshot_ids[0],
                        run_id="published-workspace-projection",
                        run_status="approved",
                    ),
                ),
                statements=(
                    NarrativeStatement(
                        text="Source-bound report section generated by the backend.",
        ...
        return self.generate(template_id, template_version, sections)
    ...
    def approve(self, report_id: str, approver: str) -> ReportArtifact:
        ...
        approved = replace(
            ...
            approval=ReportApproval(
                approver=approver.strip(), approved_at=datetime.now(UTC)
            ),
        )
        ...
        for section in sections:
            for value in section.values:
                ...
                if not value.is_approved:
                    raise ValueError(
                        f"value {value.key} does not reference an approved run"
                    )
            for statement in section.statements:
                if not statement.source_refs:
                    raise ValueError(
                        "every material statement requires source references"
                    )
...
class ReportExporter:
    ...
    def _json(report: ReportArtifact) -> bytes:
        return json.dumps(_json_ready(report), ensure_ascii=False, indent=2).encode(
            "utf-8"
        )
    ...
    def _powerpoint(report: ReportArtifact) -> bytes:
        ...
        title_slide.shapes.title.text = report.report_type.value.replace(
            "_", " "
        ).title()
        cast(
            SlidePlaceholder, title_slide.placeholders[1]
        ).text = f"Report {report.report_id}"
        for section in report.sections:
            slide = presentation.slides.add_slide(presentation.slide_layouts[1])
            assert slide.shapes.title is not None
            slide.shapes.title.text = section.title
            frame = cast(SlidePlaceholder, slide.placeholders[1]).text_frame
            frame.clear()
            lines = [
                f"{value.key}: {value.value} {value.unit or ''}"
                for value in section.values
            ]

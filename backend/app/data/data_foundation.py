...
class DataQualityReport:
    ...
    def score(self) -> float:
        if self.row_count == 0:
            return 0.0
        penalty = sum(
            10 if item.severity == FindingSeverity.ERROR else 2
...
class TabularImporter(Protocol):
    def load(
        self, content: bytes, *, column_mapping: Mapping[str, str] | None = None
    ) -> tuple[FinanceRecord, ...]: ...
        ...
        for row_number, row in enumerate(rows, start=first_data_row):
            ...
            records.append(
                FinanceRecord(
                    ...
                    scenario=str(
                        row.get(mapping.get("scenario", "scenario"), "")
                    ).strip(),
                    value=value,
                    currency=str(row.get(mapping.get("currency", "currency"), ""))
                    .strip()
                    .upper(),
    ...
    def load(
        ...
        *,
        ...
        return self.load_text(
            content.decode("utf-8-sig"), column_mapping=column_mapping
        )
        ...
        *,
        ...
        for record in materialized:
            ...
            if len(record.currency) != 3 or not record.currency.isalpha():
                ...
                allowed_currencies is not None
                and record.currency not in allowed_currencies
            ):
                findings.append(
                    DataQualityFinding(

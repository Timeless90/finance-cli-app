...
from app.data.data_foundation import (
...
from app.data.data_semantics import FinanceSemanticService, SemanticModel
...
class FinanceDataWorkflow:
    ...
    def _load(self, command: FinanceImportCommand) -> tuple[FinanceRecord, ...]:
        ...
        if normalized == "csv":
            return self._csv.load(
                command.content, column_mapping=command.column_mapping
            )

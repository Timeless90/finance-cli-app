    ...
    def simulate(self, actions: tuple[ManagementAction, ...]) -> ActionSimulationResult:
        ...
        periods = tuple(
            ActionSimulationPeriod(
                ...
                covenant_effects=tuple(
                    sorted(covenant_by_period.get(period, {}).items())
                ),
            )
        ...
        for values in covenant_by_period.values():
            for covenant_id, amount in values.items():
                covenant_totals[covenant_id] = (
                    covenant_totals.get(covenant_id, ZERO) + amount
                )
...
class ActionPortfolioPrioritizer:
    def prioritize(
        self, actions: tuple[ManagementAction, ...]
    ) -> tuple[ActionPriority, ...]:
...
class ActionReviewService:
    def review(self, action: ManagementAction, current_period: int) -> ActionReview:
        ...
        if current_period > action.due_period and action.status not in {
            ActionStatus.COMPLETED,
            ActionStatus.CANCELLED,
        }:
            return ActionReview(
...
@dataclass(frozen=True, slots=True)
class BenefitTrackingDefinition:
    """Published actuals/baselines used by a governed benefit-tracking run."""
...
class InMemoryBenefitTrackingCatalog:
    def __init__(self) -> None:
        self._definitions: dict[
            tuple[str, str, str, str], BenefitTrackingDefinition
        ] = {}

    def register(self, definition: BenefitTrackingDefinition) -> None:
        key = (
        ...
        if key in self._definitions:
            raise ValueError(
                f"benefit definition already exists: {definition.action_id}"
            )
    ...
    def get(
        ...
        *,
        ...
        try:
            return self._definitions[(company_id, period_id, scenario_id, action_id)]
        except KeyError as exc:
            raise KeyError(f"unknown benefit definition: {action_id}") from exc

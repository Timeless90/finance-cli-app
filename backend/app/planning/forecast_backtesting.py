    ...
    def evaluate(
        ...
        for observation in forecasts:
            ...
            if observation.lower is not None or observation.upper is not None:
                ...
                log_scores.append(
                    0.5 * log(2.0 * pi * sigma * sigma) + 0.5 * standardized**2
                )
        ...
        return ForecastBacktestMetrics(
            ...
            bias=mean(errors),
            coverage=mean(1.0 if item else 0.0 for item in covered)
            if covered
            else None,
    ...
    def assert_no_future_leakage(
        *,
        ...
        if training_end_index > origin_index:
            raise ValueError(
                "future leakage detected: training data extends past origin"
            )

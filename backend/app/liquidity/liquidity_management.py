...
class ThirteenWeekCashForecast:
    def forecast(
        self, flows: tuple[WeeklyCashFlow, ...]
    ) -> tuple[WeeklyCashPosition, ...]:
        if len(flows) != 13:
        ...
        for item in flows:
            ...
            if prior_close is not None and item.bank_opening != prior_close:
                raise ValueError(
                    "bank opening cash must reconcile to prior closing cash"
                )
            inflows = (
                item.ar_collections
    ...
    def forecast(
        ...
        for period in periods:
            ...
            if prior_close is not None and period.opening_cash != prior_close:
                raise ValueError(
                    "monthly opening cash must reconcile to prior closing cash"
                )
            closing = (
                opening
...
class WorkingCapitalModel:
    ...
    def calculate(
        self, assumptions: WorkingCapitalAssumptions
...
class CovenantEngine:
    ...
    def evaluate(
        ...
        if definition.direction == CovenantDirection.MAXIMUM:
            ...
            breaches = sum(value < definition.threshold for value in simulated_values)
        probability = (
            Decimal(breaches) / Decimal(len(simulated_values))
            if simulated_values
            else ZERO
        )

...
from app.governance.governance_catalog import ScenarioRepository
from app.shared.domain.value_objects import FiscalPeriod
...
class ContextCatalogService:
    ...
    def principal_view(self, principal: Principal) -> Mapping[str, Any]:
        ...
        return {
            ...
            "permissions": sorted(
                permission.value
                for permission in self._access.permissions_for(principal)
            ),
        }
    ...
    def list_scenarios(
        ...
        *,
        ...
        return tuple(
            sorted(options.values(), key=lambda item: (item.label, item.scenario_id))
        )

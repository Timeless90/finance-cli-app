    ...
    def __post_init__(self) -> None:
        ...
        if self.distribution == SeverityDistribution.EMPIRICAL:
            ...
            if self.pareto_scale <= ZERO or self.pareto_shape <= ONE:
                raise ValueError(
                    "Pareto scale must be positive and shape must exceed 1"
                )
...
class RiskQuantificationEngine:
    def expected_severity(self, model: RiskQuantification) -> Decimal:
        ...
        if model.distribution == SeverityDistribution.LOGNORMAL:
            ...
            value = exp(
                float(model.lognormal_mu) + float(model.lognormal_sigma) ** 2 / 2
            )
    ...
    def sample_severity(self, model: RiskQuantification, uniform: float) -> Decimal:
        ...
        if model.distribution == SeverityDistribution.LOGNORMAL:
            ...
            value = exp(
                float(model.lognormal_mu) + float(model.lognormal_sigma) * z_score
            )
            return Decimal(str(value))
        assert model.pareto_scale is not None
        assert model.pareto_shape is not None
        value = float(model.pareto_scale) / (
            (1 - bounded) ** (1 / float(model.pareto_shape))
        )
        return Decimal(str(value))
...
class RiskAggregationEngine:
    ...
    def aggregate(
        ...
        *,
        ...
        factor = self._cholesky(
            self._validate_correlation(correlation_matrix, len(risks))
        )
        rng = Random(seed)
        ...
        for risk, gross_values, net_values in zip(
            risks, gross_by_risk, net_by_risk, strict=True
        ):
    ...
    def _sample_annual_loss(
        ...
        if model.frequency_model == FrequencyModel.BERNOULLI:
            events = (
                1 if occurrence_uniform < float(model.occurrence_probability) else 0
            )
        else:
            events = self._poisson_from_uniform(
                float(model.annual_frequency), occurrence_uniform
            )
        total = ZERO
        ...
        for row in range(size):
            for column in range(row + 1):
                ...
                if row == column:
                    ...
                    lower[row][column] = (matrix[row][column] - subtotal) / lower[
                        column
                    ][column]
    ...
    def _check_double_count_groups(risks: tuple[RiskRecord, ...]) -> None:
        ...
        if duplicates:
            details = ", ".join(
                f"{group}={ids}" for group, ids in sorted(duplicates.items())
            )
...
class RiskPlanMapping:
    ...
    def resolved_impact_key(self) -> str:
        return (
            self.impact_key
            or f"{self.statement}:{self.metric}:{self.period}:{self.risk_id}"
        )

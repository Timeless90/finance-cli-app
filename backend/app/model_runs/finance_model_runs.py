...
from app.risk.risk_management import RiskAggregationEngine, RiskRegisterService
...
class InMemoryFinanceModelRunRepository:
    _ALLOWED_TRANSITIONS: ClassVar[dict[ModelRunStatus, frozenset[ModelRunStatus]]] = {
        ...
        with self._lock:
            ...
            if run.status is not ModelRunStatus.PENDING:
                raise ModelRunStateConflict(
                    "new model runs must start in pending state"
                )
...
class FinanceModelRunService:
    ...
    def execute(self, run_id: str) -> FinanceModelRun:
        ...
        except Exception as exc:  # noqa: BLE001 - failed runs must retain audit state
    ...
    def _execute_market_risk(self, run: FinanceModelRun) -> Mapping[str, Any]:
        ...
        if model_type is MarketRiskModelType.VAR_ES:
            ...
            if method is MarketRiskVarMethod.HISTORICAL:
                return asdict(self._market_risk_metrics.historical(losses, confidence))

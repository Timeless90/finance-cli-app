...
from app.shared.quant.interfaces import QuantModelInput, QuantModelOutput
...
class LegacyPortfolioSimulationModel:
    ...
    def execute(self, model_input: QuantModelInput) -> QuantModelOutput:
        ...
        log_returns = generate_log_returns(
            ...
            n_paths=int(cast(int, parameters["n_paths"])),
            n_months=int(cast(int, parameters["n_months"])),
            seed=seed,
            block_length=int(cast(int, parameters.get("block_length", 12))),
            student_t_df=cast(float | None, parameters.get("student_t_df")),
        )
        output = simulate_portfolio(
            log_returns,
            initial_value=float(cast(float, parameters["initial_value"])),
            monthly_contribution=float(cast(float, parameters["monthly_contribution"])),
            timing=ContributionTiming(str(parameters["contribution_timing"])),
            annual_inflation=float(
                cast(float, parameters.get("annual_inflation", 0.0))
            ),
            annual_external_fee=float(
                cast(float, parameters.get("annual_external_fee", 0.0))
            ),
        )

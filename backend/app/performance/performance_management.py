...
class KpiNode:
    ...
    def __post_init__(self) -> None:
        ...
        if overlap:
            raise ValueError(
                f"children cannot be both additive and subtractive: {overlap}"
            )
...
class ForecastAccuracyService:
    def summarize(
        self, observations: Iterable[AccuracyObservation]
    ) -> tuple[AccuracySlice, ...]:
        ...
        *,
        ...
        if commentary is not None:
            if commentary.kpi != kpi or commentary.period != period:
                raise ValueError(
                    "commentary must reference the evaluated KPI and period"
                )

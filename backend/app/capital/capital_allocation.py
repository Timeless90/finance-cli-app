...
@dataclass(frozen=True, slots=True)
class CapitalCandidateDefinition:
    """Authoritative project inputs selected by business ID, never supplied by a browser run request."""
...
@dataclass(frozen=True, slots=True)
class FundingScenarioDefinition:
    company_id: str
    period_id: str
    scenario_id: str
...
class InMemoryCapitalCandidateCatalog:
    def __init__(self) -> None:
        self._candidates: dict[
            tuple[str, str, str, str], CapitalCandidateDefinition
        ] = {}
        self._constraints: dict[tuple[str, str, str], PortfolioConstraints] = {}
        self._funding_options: dict[
            tuple[str, str, str, str], FundingScenarioDefinition
        ] = {}

    def register(self, candidate: CapitalCandidateDefinition) -> None:
        key = (
        ...
        if key in self._candidates:
            raise ValueError(
                f"capital candidate already exists: {candidate.candidate_id}"
            )
        self._candidates[key] = candidate

    def get(
        self, *, company_id: str, period_id: str, scenario_id: str, candidate_id: str
    ) -> CapitalCandidateDefinition:
        key = (company_id, period_id, scenario_id, candidate_id)
        try:
            return self._candidates[key]
        except KeyError as exc:
            raise KeyError(f"unknown capital candidate: {candidate_id}") from exc
    ...
    def register_constraints(
        ...
        *,
        ...
        key = (company_id, period_id, scenario_id)
        if key in self._constraints:
            raise ValueError("capital constraints already exist for context")
    ...
    def constraints_for(
        ...
        *,
        ...
        try:
            return self._constraints[(company_id, period_id, scenario_id)]
        except KeyError as exc:
            raise KeyError("capital constraints not found for context") from exc

    def register_funding(self, definition: FundingScenarioDefinition) -> None:
        key = (
        ...
        if key in self._funding_options:
            raise ValueError(
                f"funding option already exists: {definition.option.option_id}"
            )
        self._funding_options[key] = definition
        ...
        *,
        ...
        try:
            return self._funding_options[
                (company_id, period_id, scenario_id, option_id)
            ]
        except KeyError as exc:
            raise KeyError(f"unknown funding option: {option_id}") from exc


class ProjectValuationService:
    def evaluate(
        self, project: CapitalProject, discount_rate: float
    ) -> ProjectValuation:
...
class MonteCarloNpvEngine:
    def simulate(
        ...
        *,
        ...
        discount = np.power(
            1.0 + discount_rate, np.arange(1, len(base) + 1, dtype=float)
        )
        npvs = -project.initial_investment + np.sum(
            simulated / discount[None, :], axis=1
        )
        ...
        *,
        ...
        for size in range(len(projects) + 1):
            for subset in combinations(projects, size):
                ...
                cash = constraints.opening_cash_headroom + sum(
                    project.cash_headroom_impact - project.initial_investment
                    for project in subset
                )
                leverage = constraints.base_leverage + sum(
                    project.leverage_delta for project in subset
                )
                ...
                total_npv = sum(
                    risk_adjusted_npvs.get(project.project_id, 0.0)
                    for project in subset
                )
                score = total_npv + strategic_weight * sum(
                    project.strategic_score for project in subset
                )
                if score <= best_score:
                    continue
                ...
                best = PortfolioSelection(
                    selected_project_ids=tuple(
                        project.project_id for project in subset
                    ),
        ...
        return PortfolioSelection(
            ...
            constraints_satisfied=(
                ...
                and constraints.base_interest_cover
                >= constraints.minimum_interest_cover
            ),
        )
...
class FundingScenarioEngine:
    def evaluate(
        ...
        *,
        ...
        annual_principal = (
            option.amount / option.term_years if option.amortizing else 0.0
        )
        ...
        interest_cover_after = (
            base_ebitda / interest_after if interest_after > 0.0 else float("inf")
        )

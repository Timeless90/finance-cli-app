    ...
    def fit(self, returns: FloatArray, *, bic_improvement: float = 2.0) -> GarchFit:
        ...
        def conditional_variance(params: FloatArray) -> FloatArray:
            ...
            for index in range(1, values.size):
                h[index] = (
                    omega + alpha * centered[index - 1] ** 2 + beta * h[index - 1]
                )
        ...
        baseline_ll = float(
            np.sum(t.logpdf(centered, df=8.0, loc=0.0, scale=sqrt(variance)))
        )
        baseline_bic = 2.0 * log(values.size) - 2.0 * baseline_ll
        enabled = bic + bic_improvement < baseline_bic
        reason = (
            "BIC improves over static-volatility baseline"
            if enabled
            else "no sufficient BIC improvement"
        )
        return GarchFit(
        ...
        *,
        ...
        def emission(var: FloatArray) -> FloatArray:
            ...
            for state in range(2):
                result[:, state] = np.exp(-0.5 * centered**2 / var[state]) / sqrt(
                    2.0 * np.pi * var[state]
                )
        ...
        for _ in range(max_iter):
            ...
            transition = xi_sum / np.maximum(
                np.sum(xi_sum, axis=1, keepdims=True), 1e-12
            )
            initial = gamma[0]
            for state in range(2):
                weight = float(np.sum(gamma[:, state]))
                variances[state] = float(
                    np.sum(gamma[:, state] * centered**2) / max(weight, 1e-12)
                )
        ...
        baseline_ll = float(
            np.sum(
                -0.5
                * (np.log(2.0 * np.pi * base_variance) + centered**2 / base_variance)
            )
        )
...
class EvtTailOverlay:
    def fit(
        ...
        *,
        ...
        enabled = bool(
            np.isfinite(shape) and np.isfinite(scale) and scale > 0.0 and shape < 1.0
        )
...
class CopulaDependenceModel:
    ...
    def _pseudo_uniform(data: FloatArray) -> FloatArray:
        ...
        for column in range(columns):
            uniforms[:, column] = rankdata(data[:, column], method="average") / (
                rows + 1.0
            )
    ...
    def fit(self, returns: FloatArray, *, aic_improvement: float = 2.0) -> CopulaFit:
        ...
        if data.ndim != 2 or data.shape[0] < 200 or data.shape[1] < 2:
            raise ValueError(
                "copula fit requires at least 200 rows and two risk factors"
            )
        ...
        use_t = (
            t_aic + aic_improvement < gaussian_aic
            and max(tail_dependences, default=0.0) > 0.01
        )
        if use_t:
            return CopulaFit(
                family="student_t",
        ...
        return CopulaFit(
            ...
            correlation=tuple(
                tuple(float(item) for item in row) for row in gaussian_corr
            ),
...
class HedgeScenarioEngine:
    ...
    def optimal_ratio(
        self, exposure_returns: FloatArray, hedge_returns: FloatArray
    ) -> float:
        exposure = np.asarray(exposure_returns, dtype=float)
        hedge = np.asarray(hedge_returns, dtype=float)
    ...
    def evaluate(
        ...
        *,
        ...
        if losses.shape != forecasts.shape or losses.ndim != 1 or losses.size < 50:
            raise ValueError(
                "aligned loss and VaR arrays with at least 50 observations are required"
            )
        if not 0.5 < confidence < 1.0:
            raise ValueError("confidence must be between 0.5 and 1.0")
        ...
        observed_rate = x / n
        eps = 1e-12
        null_ll = (n - x) * log(max(1.0 - expected_rate, eps)) + x * log(
            max(expected_rate, eps)
        )
        alt_ll = (n - x) * log(max(1.0 - observed_rate, eps)) + x * log(
            max(observed_rate, eps)
        )

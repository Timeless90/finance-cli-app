...
from app.governance.governance import GovernanceStatus, _hash_payload
...
class ModelRegistration:
    ...
    approved_by: str | None = None
    created_at: datetime = datetime.min.replace(tzinfo=UTC)  # noqa: RUF009 - immutable sentinel
...
class InMemoryScenarioRepository:
    ...
    def list_all(self) -> tuple[ScenarioVersion, ...]:
        return tuple(
            sorted(
                self._items.values(), key=lambda item: (item.scenario_id, item.version)
            )
        )
...
class ScenarioService:
    ...
    def clone(
        self, scenario_id: str, *, actor: str, name: str | None = None
    ) -> ScenarioVersion:
    ...
    def compare(self, left_id: str, right_id: str) -> Mapping[str, tuple[Any, Any]]:
        ...
        return {
            key: (left_map.get(key), right_map.get(key))
...
class ModelRegistryService:
    ...
    def validate(
        self, model_id: str, version: str, *, run_ids: tuple[str, ...]
    ) -> ModelRegistration:
        if not run_ids:
            raise ValueError("at least one validation run is required")
        current = self._require(model_id, version)
        updated = replace(
            current, lifecycle=ModelLifecycle.VALIDATED, validation_run_ids=run_ids
        )

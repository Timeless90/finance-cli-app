...
class GovernedRunService:
    ...
    def create(
        ...
        *,
        ...
        self._runs.add(run)
        self._append_event(
            None, run, AuditAction.CREATED, actor, correlation_id, "run created"
        )
        return run
    ...
    def validate(self, run_id: str, *, actor: str, correlation_id: str) -> GovernedRun:
        ...
        updated = replace(
            current, status=GovernanceStatus.VALIDATED, validated_by=actor
        )
    ...
    def retire(
        self, run_id: str, *, actor: str, correlation_id: str, reason: str
    ) -> GovernedRun:
        current = self._require(run_id)
        ...
        self._runs.replace(updated)
        self._append_event(
            current, updated, AuditAction.RETIRED, actor, correlation_id, reason
        )
        return updated

...
from app.shared.rbac import AccessControlService, Permission, Principal, Role


def _service() -> tuple[
    GovernedRunService, InMemoryGovernedRunRepository, InMemoryAuditEventRepository
]:
...
def test_sqlite_run_and_audit_repositories_are_durable(tmp_path) -> None:
    ...
    service = GovernedRunService(
        SqliteGovernedRunRepository(database), SqliteAuditEventRepository(database)
    )
    ...
    assert (
        len(SqliteAuditEventRepository(database).list_for("governed_run", run.run_id))
        == 1
    )
...
def test_scenario_and_model_governance() -> None:
    ...
    assert (
        scenarios.approve(base.scenario_id, actor="reviewer").status
    ...
    assert (
        models.approve(model.model_id, model.version, actor="reviewer").lifecycle
...
def test_rbac_enforces_permissions_and_company_scope() -> None:
    ...
    principal = Principal(
        "controller", frozenset({Role.CONTROLLER}), frozenset({"DE01"})
    )
...
def test_governance_api_run_lifecycle() -> None:
    ...
    assert (
        client.post(
            f"/api/v1/governance/runs/{run_id}/validate",
            headers={"x-user": "validator", "x-roles": "risk"},
        ).status_code
        == 200
    )
    assert (
        client.post(
            f"/api/v1/governance/runs/{run_id}/approve",
    ...
    lineage = client.get(
        ...
        headers={"x-user": "auditor", "x-roles": "reviewer"},
    )
    assert lineage.status_code == 200
    assert lineage.json()["snapshot_id"] == "snapshot"


def test_governance_api_uses_configured_sqlite_store_after_restart(
    tmp_path: Path,
) -> None:
    database = tmp_path / "governance.db"

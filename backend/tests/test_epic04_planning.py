...
def test_integrated_statements_reconcile() -> None:
    ...
    assert result.closing_cash == (
        Decimal(1000) + result.operating_cash_flow + result.investing_cash_flow
    )
...
def test_multi_period_roll_forward_uses_prior_closing_balances() -> None:
    ...
    periods = tuple(
        _period(f"2027-{month:02d}", opening_cash="0") for month in range(1, 13)
    )
    ...
    assert all(
        result.balance_sheet_difference == Decimal(0) for result in forecast.results
    )

...
def test_monte_carlo_aggregation_is_reproducible_and_traceable() -> None:
    ...
    assert sum(
        (item.expected_loss_share for item in first.contributions), Decimal(0)
    ) == pytest.approx(Decimal(1), abs=Decimal("0.0000000001"))
...
def test_reporting_contains_top_risks_heatmap_methodology_and_mitigation() -> None:
    ...
    portfolio = RiskAggregationEngine().aggregate(
        risks, correlations, paths=1_000, seed=7
    )

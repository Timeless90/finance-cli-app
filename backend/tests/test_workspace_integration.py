...
from app.governance.governance_catalog import Assumption, ScenarioKind
from app.shared.composition import build_container
from app.shared.rbac import Principal, Role
from app.workspace.workspace_integration import CommandCenterSnapshot
...
def test_command_center_returns_only_published_backend_read_models() -> None:
    ...
    container.workspace_read_model_service.publish_command_center(
        CommandCenterSnapshot(
            context=context,
            as_of=datetime(2026, 8, 8, 9, 0, tzinfo=UTC),
            metrics=(
                {
                    "metric_id": "ebitda",
                    "label": "EBITDA",
            ...
            variance_drivers=({"driver": "price", "impact": 8.0},),
            actions=({"action_id": "A-1", "status": "open"},),
